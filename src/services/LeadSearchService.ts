...
export class LeadSearchService {
  static async searchLeads(searchType: 'badWebsites' | 'criteria', criteria: SearchCriteria): Promise<DiscoveredLead[]> {
    try {
      console.log('Calling lead search function with:', { searchType, criteria });
      ...
      if (!data?.leads) {
        console.warn('No leads returned from search');
        return [];
      }
      ...
      console.error('Lead search service error:', error);
      throw error;
    }
  }
}